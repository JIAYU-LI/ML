...
class AdamLearningRule(GradientDescentLearningRule):
    def __init__(self, learning_rate=1e-3, const=1e-8, p1=0.9, p2=0.999):
        ...
        assert const != 0., ('const should not be zero.')
        self.learning_rate = learning_rate
        self.const = const
        self.p1 = p1
        self.p2 = p2
    ...
    def initialise(self, params):
        ...
        self.t = 0
        for param in self.params:
            self.momrs.append(np.zeros_like(param))
        for param in self.params:
            self.moms.append(np.zeros_like(param))
        for param in self.params:
            self.t += 1 
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, momr, mom, grad in zip(self.params, self.momrs, self.moms, grads_wrt_params):
            mom *= self.p1
            mom += (1-self.p1)*grad
            mom2 = mom/(1-self.p1**self.t)
            momr *= self.p2
            momr += (1-self.p2)*grad**2
            momr2 = momr/(1-self.p2**self.t)
            param -= self.learning_rate * mom2/np.sqrt(self.const+momr2)
            
class RMSPropLearningRule(GradientDescentLearningRule):
    def __init__(self, learning_rate=1e-3, const=1e-8, p=0.9):
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, momr, grad in zip(self.params, self.momrs, grads_wrt_params):
            momr *= self.p
            momr += (1-self.p)*(grad**2)
            param -= self.learning_rate * grad/np.sqrt(self.const+momr)
