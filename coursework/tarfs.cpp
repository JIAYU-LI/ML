...
int TarFSFile::pread(void* buffer, size_t size, off_t off)
{
	...
	//Get the owning file's total block number.
	size_t nr_blocks = _owner.block_device().block_count();

	//Build a new buffer "add" with this file size.
	uint8_t *add = new uint8_t[512 * nr_blocks];

	//The maximum byte number from off in the offset to the end of this file.
        size_t size2 = (nr_blocks - _file_start_block)*512 + (511 - off)-1;

	//Read data blocks of this file into the new buffer.
	 _owner.block_device().read_blocks(add,_file_start_block, nr_blocks);

	//First case: required size is less than the maximum byte number.
	syslog.messagef(LogLevel::DEBUG, "Block Device fisrt_block=%lu", _file_start_block);

	if (size2 >= size) {
	  //Copy the content of the new buffer add with byte size into the required buffer.
	  memcpy(buffer, add+off, size); 
	   return size;
	}
	//Second case: required size is larger than the maximum byte number and it spans another file.
	else {
	  //Only byte number within the file can be read into buffer.
	  memcpy(buffer, add+off, size2); 
	  return size2;
	}
...
TarFSNode* TarFS::build_tree()
{
	...
	//Iterate over all header blocks in this TAR file.
	while (sum < nr_blocks){
	// Allocate storage for the header.
	posix_header * header = (struct posix_header *) new char[block_size * 1];
	
	
	//Read head block into the new header and try to find where the next header block is.
	block_device().read_blocks(header, sum, 1);
	unsigned int num_size = octal2ui(header->size);
	unsigned int n = num_size/512 + 1;
	sum += n;

	//Get filename or directoryname from the header block then split it into a list.	
	String path = header->name;

	List<String> lists_children = path.split('/', true);
	int num_children = lists_children.count();
	//Frist case: only 1 child under the root node and conduct different operation according to its type.
	if (num_children == 1){
		if (header->typeflag == '0'){
		
		TarFSNode *node = new TarFSNode(root, lists_children.at(0), *this);
		
	        root->add_child(lists_children.at(0), node);
		node->size(num_size);
		//node->set_block_offset(_block_offset);
				}
	...
	//Second case: the node under root node is a directory and it has child.
	else{
		TarFSNode *pre_node = root; //I hope to judge whether child name is already in the tree using interation.
		for(int i =0; i<num_children;i++){
			
			TarFSNode *pre_node = new TarFSNode(pre_node, lists_children.at(i), *this);

			pre_node->add_child(lists_children.at(i), pre_node);
			 
			}
		}
	lists_children.clear();	//clear list after node building.
	delete header; //clear header in order to create new one to store the next header block.
	}
	return root;
}
...
{
	// Get size info from the header block then convert it to int
