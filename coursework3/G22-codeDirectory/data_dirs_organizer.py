...
from data_provider64 import TRAIN_DIR, TEST_DIR, DATA_DIR
from data_provider64 import load_train_info
from data_provider64 import save_organized_data_info, load_organized_data_info
...
def _load_paths_labels_from_train_dir():
    ...
    paths, labels = [], []          #create lists in order to store 'path' and 'label'
...
#this stratified ShuffleSplit cross-calidator provides train/test indices to split data in train/test sets then returns stratified randomized flods.
def _train_val_split_indices(labels):    
#the flods compose of preserving the percentage of samples for each class                           
...
#state each elements in the 'info'
    info = {
        'dir_tr': dir_tr,
        'num_tr': num_tr,
        'dir_val': NEW_VAL_DIR,
...
def _save_images_to_dir(
        dest_dir, src_paths, labels, distinct_classes, multi_crop=False):
